...
export default function PaginationItem({ pagination, theme }: any) {
    ...
    return <Row className='pb-10 mb-5'>
        <Col md={{ span: 12 }} span={24}>
            <div>
                {pagination.next && <Link className={theme ? Styles.lightLink : Styles.darkLink} href={`/${pagination.next?.id}`}>
        ...
        <Col md={{ span: 12 }} span={24} className="md:text-right md:pt-0 pt-8">
            <div>
                {pagination.prev && <Link className={theme ? Styles.lightLink : Styles.darkLink} href={`/${pagination.prev?.id}`}>
